...
export function EstadisticasClient({ stats }: EstadisticasClientProps) {
  return (
    <div className="container max-w-4xl py-8 px-4">
      <div className="mb-6">
        <Button variant="light" size="sm" as={Link} href="/perfil" className="gap-2">
      ...
      {stats.totalQuizzes === 0 ? (
        <Card>
          <CardBody className="py-12 text-center">
            ...
            <Button color="primary" as={Link} href="/salsa/quiz">
              Comenzar un quiz
            </Button>
          </CardBody>
        </Card>
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <Card>
              <CardHeader className="flex flex-row items-center gap-2">
                ...
                <p className="text-lg font-semibold">Por género</p>
              </CardHeader>
              <CardBody>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center gap-2">
                ...
                <p className="text-lg font-semibold">Por modo</p>
              </CardHeader>
              <CardBody>
                <div className="space-y-3">
                  <ModeRow
          ...
          {stats.mostMissedProgressions.length > 0 && (
            <Card>
              <CardHeader className="flex flex-row items-center gap-2">
                ...
                <p className="text-lg font-semibold">Áreas de mejora</p>
              </CardHeader>
              <CardBody>
          ...
          {stats.recentQuizzes.length > 0 && (
            <Card>
              <CardHeader>
                <p className="text-lg font-semibold">Quizzes recientes</p>
              </CardHeader>
              <CardBody>
                <div className="space-y-3">
                  {stats.recentQuizzes.map((quiz) => (
                    ...
                    >
                      <div>
                        <p className="font-medium">{quiz.genreName || 'Desconocido'}</p>
                        <p className="text-xs text-muted-foreground">
                          {quiz.mode === 'piano' ? 'Piano' : 'Repertorio'} ·{' '}
                          {quiz.completedAt
                            ? new Date(quiz.completedAt as Date).toLocaleDateString('es-ES', {
                                day: 'numeric',
                                month: 'short',
                              })
                            : 'En progreso'}
