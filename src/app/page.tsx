  ...
  return (
    <div className="max-w-4xl mx-auto py-16">
        ...
        {genresWithStats.map((genre) => {
          if (!genre.hasContent) {
            return (
              <div
                key={genre.id}
                className="relative overflow-hidden p-6 rounded-3xl bg-card/30 border border-border/30 opacity-60"
              >
          ...
          return (
            <Link
              key={genre.id}
              href={`/${genre.id}`}
              className="group relative overflow-hidden p-6 rounded-3xl bg-card/50 border border-border/50 hover:border-primary/50 transition-all duration-300 hover:glow"
            >
              <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              <div className="relative">
                <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center mb-4 group-hover:bg-primary/20 group-hover:glow-sm transition-all duration-300">
      ...
      <div className="mt-20 text-center">
        <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-secondary/50 border border-border/50 text-sm text-muted-foreground">
